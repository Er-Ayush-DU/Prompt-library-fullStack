...
//             avatarUrl: user.avatarUrl,
//             bio: user.bio,
//             isAdmin: user.isAdmin || false   // ← YE LINE ADD KI (sirf yeh!)
//           };
//         } catch (error) {
//           throw new Error("Error during authentication: " + (error instanceof Error ? error.message : "Unknown error"));
//         }
...
//         token.isAdmin = (user as any).isAdmin;   // ← YE LINE ADD KI
//       }
//       console.log("JWT callback token:", token);
//       return token;
//     },
...
//         // ← YE 2 LINES ADD KI (isAdmin session mein bhi aayega)
...
//     // ← YE PURA CALLBACK ADD KIYA (redirect ke liye)
...
// // Helper function (tera wahi rahega)
// export async function getUserFromCookies() {
//   const session = await getServerSession(authOption);
//   if (!session?.user) return null;
...
//     avatarUrl: session.user.avatarUrl,
//     bio: session.user.bio,
//     isAdmin: (session.user as any).isAdmin   // ← optional, agar kahin use karega
//   };
// }